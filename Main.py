...
def markAttendance(name, new_id=None):
    ...
    with open("Attendance.csv", 'r+') as f:
        ...
        if name not in attend_dict.keys():
            ...
            if new_id is None:
                new_id = pyautogui.prompt(f"Enter ID for {name}", title="Enter Details")
            attend_dict[name] = [new_id, dtString, "Present"]
            ...
            # Check if re-entered ID is different
            while new_id is not None and new_id != attend_dict[name][0]:
                tk.messagebox.showinfo("Wrong ID", "Incorrect ID. Please re-enter the correct ID.")
                new_id = pyautogui.prompt(f"Re-enter ID for {name}", title="Re-enter ID")

            attend_dict[name][1] = dtString
            attend_dict[name][2] = "Present"
# ...
...
def delete_from_database():
    ...
    for xx in filesz:
        os.remove(xx)
        xx = os.path.splitext(os.path.basename(xx))[0]
...
def deregister():
    ...
    root1.title("Delete")
    image2 = PhotoImage(file=r"C:\Games\Project\Attendance Automation Using Facial Recognition\icons and "
                             r"background\remove.png")
...
# GUI Initialization
imag = tk.PhotoImage(
    file=r"C:\Games\Project\Attendance Automation Using Facial Recognition\icons and "
         r"background\wp8047222-blue-tech-wallpapers.png")
